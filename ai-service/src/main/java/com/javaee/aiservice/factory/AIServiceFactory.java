...
public class AIServiceFactory {
        ...
        for (ModelType modelType : ModelType.values()) {
            ...
            if ("dashscope".equals(provider)) {
                ...
                if (multiModelConfig.getOpenai().containsKey(modelType.getCode())) {
                    ...
                    enabled = config.isEnabled();
                    if (config.getModelAlias() != null && !config.getModelAlias().isEmpty()) {
                        modelAlias = config.getModelAlias();
                    }
                }
                
                if (modelAlias == null || modelAlias.isEmpty()) {
                    modelAlias = modelType.getSiliconflowModel();
                }
                ...
                OpenAIAIService service = new OpenAIAIService(apiKey, baseUrl, modelType, modelAlias, enabled);
                serviceMap.put(modelType, service);
                log.info("注册OpenAI兼容模型: {} -> {} (enabled: {})", modelType.getName(), modelAlias, enabled);
            }
        }
    ...
    public AIService getService(ModelType modelType) {
        ...
        if (service == null) {
            log.warn("模型 {} 未注册，尝试使用默认模型", modelType.getName());
            return getDefaultService();
        }
        if (!service.isAvailable()) {
            log.warn("模型 {} 已禁用，自动切换到默认模型", modelType.getName());
            return getDefaultService();
        }
        return service;
    }
